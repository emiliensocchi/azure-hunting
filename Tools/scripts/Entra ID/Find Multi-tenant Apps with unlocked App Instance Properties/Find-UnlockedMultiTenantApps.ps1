    ...
    if (($AppOwnerOrganizationId -ne $microsoftTenantId) -And ($AppOwnerOrganizationId -ne $tenantId) -And ($signInAudience -eq $multiTenantAudience)) {
        ...
        try {
            ...
            if ($createdKeyCredential) {
                ...
                Remove-MgServicePrincipalKey -ServicePrincipalId $servicePrincipalId -KeyId $credential.KeyId -Proof $proofOfPossession -ErrorAction Stop # this breaks sometimes
